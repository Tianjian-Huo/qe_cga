...
class QE(Abinitio):
    ...
    @staticmethod
    def get_config_path(folder_name):
        """获取 config.ini 文件路径"""
        parent_dir = os.path.dirname(os.path.abspath(folder_name))
        return os.path.join(parent_dir, "config.ini")

    # 先获取一个默认的 config.ini 路径，避免 NameError
    default_config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "config.ini"))
    
    print(f"[调试] 读取的 config.ini 绝对路径: {default_config_path}")
    
    config = configparser.ConfigParser()
    if os.path.exists(default_config_path):
        config.read(default_config_path)
        print("[调试] 读取的 config.ini 内容:")
